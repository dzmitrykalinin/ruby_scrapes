...
(cars_num/20.0).ceil.times do |page_num|
	...
	cars.push(page.css('div.b-listing-item').map { |x| x.css("div.b-listing-item-main").css("a").text.split(' ').join(' ')}).flatten!
	links.push(page.css('div.b-listing-item').map{ |x| " http://av.by/public/#{x.css("div.b-listing-item-image").css("a").attr("href").value}	"}).flatten!
	costs.push(page.css('div.b-listing-item').map{ |x| x.css('strong').text}).flatten!
end
